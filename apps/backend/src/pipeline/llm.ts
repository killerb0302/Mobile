 ...
 * yet (that's Phase 2) - this phase just needs a plain, short spoken reply
 * given the objective and the conversation so far.
 */
export async function getNextReply(objective: string, transcript: TranscriptEntry[]): Promise<LlmTurnResult> {
  ...
  const response = await getClient().messages.create({
    ...
    // or adaptive-thinking-low-effort actually gives better real-world turn
    // latency for this model - this is a build-time experiment per the plan.
